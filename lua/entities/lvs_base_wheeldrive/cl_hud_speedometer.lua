LVS:AddHudEditor( "Tach",  ScrW() - 310, ScrH() - 260,  300, 250, 300, 250, "TACH",
...
function ENT:GetBakedTachMaterial( MaxRPM )
	...
	cam.Start2D()
		...
		for i = 0, Steps do
			local Ang = AngleStep * i + startAngle
...
local TachNeedleRadiusOuter = 140
...
function ENT:LVSHudPaintTach( X, Y, w, h, ScrX, ScrY, ply )
	...
	local Ang = startAngle + (endAngle - startAngle) * (CurRPM / MaxRPM)
