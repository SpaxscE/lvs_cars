...
function ENT:AimTurret()
	...
	local AimAngles = self:WorldToLocalAngles( (self:LocalToWorld( Vector(-63,0,85) ) - self:GetEyeTrace().HitPos):Angle() )
	...
	self:SetPoseParameter("turret_pitch", self:GetTurretPitch() - 10 )
	self:SetPoseParameter("turret_yaw", self:GetTurretYaw() + 180 )
end
